...
function updateModels() {
    ...
    let selectedMake = document.getElementById("selectVehicleMake").value;
    
    // Filter the cars array to get models of the selected make
    let models = cars.filter(car => car.make === selectedMake).map(car => car.model);

    // Get the model selection dropdown element
    let modelDropdown = document.getElementById("selectVehicleModel");

    // Clear previous options
    modelDropdown.innerHTML = "";

    // Populate the model selection dropdown with the filtered models
